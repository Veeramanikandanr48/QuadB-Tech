        ...
        <div className="row">
          <div className="col-md-6 mb-4">
            <div className="poster-section">
              <img src={show.image?.original || defaultImageUrl} alt={show.name} className="img-fluid rounded" />
            </div>
          </div>
          <div className="col-md-6">
            <div className="show-details border rounded-lg p-4">
              <h2 className="show-name font-semibold text-xl mb-3">{show.name}</h2>
              <div className="rating mb-3">⭐️{show.rating?.average || 'N/A'}</div>
              ...
              <div className="genres flex flex-wrap gap-2 mb-3">
                {show.genres && show.genres.length ? (
                  show.genres.map((genre, index) => (
                    <div key={index} className="genre bg-black text-white px-3 py-1 m-2 font-semibold rounded">
                      {genre}
                    </div>
                  ))
                ) : (
              ...
              <p className="mb-3">
                <strong>Runtime:</strong> {show.runtime ? `${show.runtime} minutes per episode` : 'N/A'}
              </p>
              <p className="mb-3">
                <strong>Premiered:</strong> {show.premiered || 'N/A'}
              </p>
              {show.officialSite ? (
                <Button variant="primary" className="mb-3" href={show.officialSite} target="_blank" rel="noopener noreferrer">
                  Official Site
                </Button>
              ) : null}
              {show.schedule ? (
              ...
              <button className="btn btn-success rounded p-3 w-100 font-semibold" onClick={handleBookTicket}>
                Book now
              </button>
            </div>
          </div>
            ...
            <Modal.Body>
              <Form>
                <Form.Group controlId="formName">
                  ...
                  <Form.Control type="text" placeholder="Enter your name" name="name" onChange={handleInputChange} />
                </Form.Group>
                <Form.Group controlId="formEmail">
                  <Form.Label>Email</Form.Label>
                  <Form.Control type="email" placeholder="Enter your email" name="email" onChange={handleInputChange} />
                </Form.Group>
              </Form>
            </Modal.Body>
            <Modal.Footer>
